...
//
//
//                 // 새로운 FFT 스펙트로그램 이미지를 세 번째 버블에 업데이트
//                 if (data.melSpectrogram) {
//                     // 캐시를 방지하기 위해 URL에 타임스탬프 추가
//                     document.getElementById('melSpectrogram').src = data.melSpectrogram + '?t=' + new Date().getTime();
//                 } else {
        ...
        fetch('/scenario/sc', {
            ...
            .then(data => {
                ...
                if (data.melSpectrogram) {
                    ...
                    // 타임스탬프를 추가하여 캐시 방지
                    newImg.src = `${data.melSpectrogram}?t=${new Date().getTime()}`;
