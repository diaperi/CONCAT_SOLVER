...
public class GPTService {
    ...
    // API 키를 코드에 직접 삽입
    private final String apiKey = "sk-proj-VkwIsCz3wYHVv48cHSrpT3BlbkFJBurcyg1NTvLVnsCdby50";
...
//    // Base64 문자열을 이미지 파일로 디코딩하여 저장하는 메서드 - 디코딩
//    public void decodeBase64ToImage(String base64Data, String outputPath) {
//        try {
...
//        } catch (IOException e) {
//            logger.error("이미지 디코딩 중 오류 발생: {}", e.getMessage());
//            throw new RuntimeException("이미지 디코딩 중 오류가 발생했습니다: " + e.getMessage());
//        }
//    }
    ...
    public String getFeedback(String sttResult, String melSpectrogramPath) {
        ...
        // 프롬프트 생성
        String prompt = String.format("다음 문장에 대한 피드백을 제공하세요: %s\n문장 피드백과 연결해서 FFT 스펙트로그램 (Base64)에 대한 피드백을 제공하세요.: %s", sttResult, melSpectrogramBase64.substring(0, 1000));
    ...
    // 이미지 크기에 따라 max_tokens 계산하는 메서드
    private int calculateMaxTokensBasedOnImageSize(String imagePath) {
        try {
            BufferedImage image = ImageIO.read(new File(imagePath));
            int width = image.getWidth();
            int height = image.getHeight();

            // 이미지의 크기 비율에 따라 토큰 수 조정
            ...
            logger.error("이미지 크기 계산 중 오류 발생: {}", e.getMessage());
            return 300;
        }
    }
}
