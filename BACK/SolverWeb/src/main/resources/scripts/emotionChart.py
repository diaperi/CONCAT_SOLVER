...
# AWS S3 설정
# boto3.client 대신 boto3.resource를 사용하면 더 직관적일 수 있습니다.
...
def get_transcript_from_s3(bucket_name, key):
    try:
        ...
        transcript_content = obj.get()['Body'].read().decode('utf-8')
        return transcript_content
    except Exception as e:
        print(f"S3에서 텍스트를 가져오는 중 오류 발생: {e}")
        sys.exit(1)
...
if __name__ == "__main__":
    # 입력 인수 처리
    bucket_name = sys.argv[1]  # S3 버킷 이름
    key = sys.argv[2]  # S3 파일 경로
    participant = sys.argv[3]  # 예: "참여자1"
    output_path = sys.argv[4]  # 그래프를 저장할 경로
